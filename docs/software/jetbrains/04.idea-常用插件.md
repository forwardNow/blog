<!--#region
@author 吴钦飞
@email wuqinfei@qq.com
@create date 2025-04-19 13:38:00
@modify date 2025-12-07 10:31:51
@desc [description]
#endregion-->

# idea - 常用插件

## idea-JRebel

介绍:

* 热部署

### 2025-11-18 授权服务器

下载: https://github.com/yu-xiaoyao/jrebel-license-active-server/releases

运行: jrebel-license-active-server-windows_amd64.exe

激活:
* http://127.0.0.1:12345/{GUID}
* wuqinfei@qq.com

参考: [Jrebel使用详情：热部署神器从安装到进阶全指南](https://blog.csdn.net/weixin_40608446/article/details/154794930)

### 旧版

步骤:

1.插件市场安装 “ JRebel and XRebel ” 

2.下载 [ReverseProxy_windows_386.exe v1.4](https://github.com/ilanyu/ReverseProxy/releases/download/v1.4/ReverseProxy_windows_386.exe)

3.运行 ReverseProxy_windows_386.exe

4.在 [Generate GUIDs online](https://www.guidgen.com/) 生成一个 GUID —— `{GUID}`

5.激活
* http://127.0.0.1:8888/{GUID}
* wuqinfei@qq.com

6.安装 “ JRebel mybatisPlus extension ”

参考:

* [IDEA配置JRebel热部署（超级详细图文教程）](https://blog.csdn.net/qq_59961910/article/details/142378452)
* [Jrebel热部署启动插件的激活使用教程](https://zhuanlan.zhihu.com/p/668387437)

## MybatisX

说明:

* 支持 mapper.xml 和 Mapper接口 之间方法的互相导航跳转
* 可以自定义代码生成器模板

参考: 

* [idea mybatis-plus之MybatisX插件小知识（代码生成 哦）](https://cloud.tencent.com/developer/article/2467427)

## MyBatisPlus

说明:

* MyBatisPlus 代码生成器