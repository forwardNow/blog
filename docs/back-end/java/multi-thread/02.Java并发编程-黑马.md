...
@modify date 2026-02-13 20:02:49
@desc [description]
#endregion-->
...
// 提交 tasks 中所有任务，带超时时间。超时针对的是 任务集合，而非单个任务
...
public class Test07 {
    public static void main(String[] args) throws Exception {
        ...
        // 等待任务执行完毕
        boolean executedFinished = pool.awaitTermination(3, TimeUnit.SECONDS);

        log.debug("执行完毕: {}", executedFinished);
        log.debug("other...");
    }
    //=> 09:10.195 [main] - shutdown
    //=> 09:10.195 [pool-1-thread-2] - begin 2
    //=> 09:10.195 [pool-1-thread-1] - begin 1
    //=> 09:10.197 [pool-1-thread-2] - 睡眠 1 秒
    //=> 09:10.197 [pool-1-thread-1] - 睡眠 1 秒
    //=> 09:11.204 [pool-1-thread-1] - end 1
    //=> 09:11.204 [pool-1-thread-2] - end 2
    //=> 09:11.204 [pool-1-thread-2] - begin 3
    //=> 09:11.204 [pool-1-thread-2] - 睡眠 1 秒
    //=> 09:12.218 [pool-1-thread-2] - end 3
    //=> 09:12.218 [main] - 执行完毕: true
    //=> 09:12.220 [main] - other...
}
```
...
public class Test08 {
    ...
    //=> 16:07.688 [main] - shutdown
    //=> 16:07.688 [pool-1-thread-1] - begin 1
    //=> 16:07.688 [pool-1-thread-2] - begin 2
    //=> 16:07.691 [pool-1-thread-1] - 睡眠 1 秒
    //=> 16:07.689 [main] - 未执行的任务: [java.util.concurrent.FutureTask@71a794e5]
    //=> 16:07.691 [pool-1-thread-2] - 睡眠 1 秒
    //=> 16:07.692 [main] - other...
    //=> 16:07.692 [pool-1-thread-2] - 异常: sleep interrupted
    //=> 16:07.692 [pool-1-thread-1] - 异常: sleep interrupted
    //=> 16:07.692 [pool-1-thread-2] - end 2
    //=> 16:07.692 [pool-1-thread-1] - end 1
}
```
...
固定大小线程池会有饥饿现象，由于线程数不足导致的

* 线程池中有两个线程，他们都可以 处理点餐 和 后厨做菜
* 必须先点餐，等才做完，才可以上菜
* 比如来了一个客人
   * 工人A 处理点餐任务，此时 工人B 等待
   * 工人A 处理完点餐，交给 工人B 做菜，此时 工人A 等待
   * 等 工人B 做完菜，上菜
* 如果同时来了两个客人
   * 工人A 处理 客人1 的点餐任务
   * 工人B 处理 客人2 的点餐任务
   * 工人A 处理完点餐，等待 工人B 去做菜
   * 工人B 处理完点餐，等待 工人A 去做菜
   * 此时没有人做饭，发生饥饿现象
...
public class Test09 {
    ...
    // 24:44.017 [pool-1-thread-1] - 处理点餐...
    // 24:44.017 [pool-1-thread-2] - 处理点餐...
}
...
public class Test10 {
    ...
    //=> 30:34.436 [pool-1-thread-1] - 处理点餐...
    //=> 30:34.439 [pool-2-thread-1] - 做菜
    //=> 30:34.439 [pool-1-thread-1] - 上菜: 烤鸡翅
    //=> 30:34.440 [pool-1-thread-1] - 处理点餐...
    //=> 30:34.440 [pool-2-thread-1] - 做菜
    //=> 30:34.440 [pool-1-thread-1] - 上菜: 地三鲜
}
```
...
* 当你执行业务计算时，这时候会使用 CPU 资源
* 当你执行 I/O 操作时、远程 RPC 调用时，包括进行数据库操作时，这时候 CPU 就闲下来了
...
public class Test11 {
    ...
    //=> 54:17.855 [Timer-0] - task 1
    //=> 54:17.858 [Timer-0] - 睡眠 2 秒
    //=> 54:19.868 [Timer-0] - task 2
}
```
...
public class Test12 {
    ...
    //=> 33:14.563 [main] - start
    //=> 33:15.570 [pool-1-thread-2] - task 2 - begin
    //=> 33:15.570 [pool-1-thread-1] - task 1 - begin
    //=> 33:15.570 [pool-1-thread-2] - 睡眠 1 秒
    //=> 33:15.570 [pool-1-thread-1] - 睡眠 1 秒
    //=> 33:16.576 [pool-1-thread-2] - task 2 - end
    //=> 33:16.576 [pool-1-thread-1] - task 1 - end
}
```

#### 8.6.3. 重复执行 - 固定频率

第 1 秒执行第 1 次任务，第 3 秒执行 第 2 次任务；如果任务耗时大于 2 秒，则第 2 次任务马上执行。

每隔 2 秒执行一次，如果 任务执行时间超过 2 秒，则下一次马上执行
...
public class Test13 {
    ...
    //=> 49:11.468 [main] - start
    //=> 49:12.469 [pool-1-thread-1] - task 1 - begin
    //=> 49:12.470 [pool-1-thread-1] - 睡眠 3 秒
    //=> 49:15.472 [pool-1-thread-1] - task 1 - end
    //=> 49:15.473 [pool-1-thread-1] - task 1 - begin
    //=> 49:15.473 [pool-1-thread-1] - 睡眠 3 秒
    //=> 49:18.486 [pool-1-thread-1] - task 1 - end
}
```

#### 8.6.4. 重复执行 - 固定延时

执行完第 N 次任务后，延迟 2 秒 再执行第 N+1 次任务
...
public class Test14 {
    ...
    //=> 53:10.284 [main] - start
    //=> 53:11.288 [pool-1-thread-1] - task 1 - begin
    //=> 53:11.288 [pool-1-thread-1] - 睡眠 3 秒
    //=> 53:14.289 [pool-1-thread-1] - task 1 - end
    //=> 53:16.298 [pool-1-thread-1] - task 1 - begin
    //=> 53:16.298 [pool-1-thread-1] - 睡眠 3 秒
    //=> 53:19.307 [pool-1-thread-1] - task 1 - end
}
```
    ...
    public static void main(String[] args) {
        ...
        Future<String> future1 = pool.submit(() -> {
            try {
                ...
                log.error("task 1 异常: {}", e.getMessage());
                return "task 1 error";
            }
        });
        //=> 12:18.304 [pool-1-thread-1] - task 1 begin
        //=> 12:18.306 [pool-1-thread-1] - task 1 异常: / by zero


        // 处理方式 2: 通过 future.get() 处理任务的异常
        Future<String> future2 = pool.submit(() -> {
            log.debug("task 2 begin");
            int i = 1 / 0;
            log.debug("task 2 end");
        ...
        try {
            ...
            log.debug("task 2 result: {}", result);
        } catch (Exception e) {
            log.error("task 2 异常: {}", e.getMessage());
        }
        //=> 14:00.738 [pool-1-thread-1] - task 2 begin
        //=> 14:00.738 [main] - task 2 异常: java.lang.ArithmeticException: / by zero
    }
}
```
...
public class Test16 {
    public static void main(String[] args) {
        ...
        // 本周四 18:00 的时间
        LocalDateTime target = now.with(DayOfWeek.THURSDAY)
                .withHour(18)
                .withMinute(0)
                .withSecond(0)
                .withNano(0);
    ...
    //=> 45:30.926 [main] - now: 2026-02-08T22:45:30.925
    //=> 45:30.928 [main] - target: 2026-02-12T18:00
    //=> 45:30.929 [main] - initialDelay: 328469075
}
```

#### 8.6.7. tomcat - 线程池

![image-20260209121900237](./images/image-20260209121900237.png)
...
Tomcat 线程池扩展了 ThreadPoolExecutor，行为稍有不同，如果总线程数达到 maximumPoolSize
...
![image-20260209124551952](./images/image-20260209124551952.png)

Tomcat 对线程池进行修改，如果核心线程满了，先创建应急线程，等应急线程也满了，最后才将任务添加进队列
...
所谓的任务拆分，是将一个大任务拆分为算法上相同的小任务，直至不能拆分可以直接求解。跟递归相关的一些计算，如 归并排序、斐波那契数列 都可以用分治思想进行求解
...
public class Test17 {
    public static void main(String[] args) {
        ...
        log.debug("{}", pool.invoke(new AddTask(5)));
    }
    //=> 46:12.890 [ForkJoinPool-1-worker-0] - fork: 2 + 【1】
    //=> 46:12.890 [ForkJoinPool-1-worker-3] - fork: 3 + 【2】
    //=> 46:12.893 [ForkJoinPool-1-worker-0] - join: 1
    //=> 46:12.890 [ForkJoinPool-1-worker-2] - fork: 4 + 【3】
    //=> 46:12.890 [ForkJoinPool-1-worker-1] - fork: 5 + 【4】
    //=> 46:12.893 [ForkJoinPool-1-worker-0] - join: 2 + 【1】 = 3
    //=> 46:12.894 [ForkJoinPool-1-worker-3] - join: 3 + 【2】 = 6
    //=> 46:12.894 [ForkJoinPool-1-worker-2] - join: 4 + 【3】 = 10
    //=> 46:12.894 [ForkJoinPool-1-worker-1] - join: 5 + 【4】 = 15
    //=> 46:12.894 [main] - 15
}
...
class AddTask extends RecursiveTask<Integer> {
    ...
    */ 
    ...
    protected Integer compute() {
        ...
        // 拆分任务（fork）
        AddTask newTask = new AddTask(n - 1);

        // 交给另一个线程去执行
        newTask.fork();

        log.debug("fork: {} + {}", n, newTask);

        // 合并结果（join）
        int result = n + newTask.join();

        log.debug("join: {} + {} = {}", n, newTask, result);
...
class AddTask extends RecursiveTask<Integer> {
    ...
    /*
    AddTask(1, 5)
        AddTask(1, 3) + AddTask(4, 5)
            AddTask(1, 2) + AddTask(3, 3) + AddTask(4, 5)
            3               3               9
    */ 
    @Override
    protected Integer compute() {
        ...
        // 拆分任务（fork）
        AddTask newTask1 = new AddTask(begin, mid);
        newTask1.fork();
        AddTask newTask2 = new AddTask(mid + 1, end);
        newTask2.fork();

        log.debug("fork: {} + {}", newTask1, newTask2);

        // 合并结果（join）
        int result = newTask1.join() + newTask2.join();

        log.debug("join: {} + {} = {}", newTask1, newTask2, result);

        return result;
    }
}
// 09:16.698 [ForkJoinPool-1-worker-3] - join: 4 + 5 = 9
// 09:16.698 [ForkJoinPool-1-worker-0] - join: 1 + 2 = 3
// 09:16.698 [ForkJoinPool-1-worker-2] - fork: 【1, 2】 + 【3, 3】
// 09:16.698 [ForkJoinPool-1-worker-1] - fork: 【1, 3】 + 【4, 5】
// 09:16.702 [ForkJoinPool-1-worker-3] - join: 3
// 09:16.702 [ForkJoinPool-1-worker-2] - join: 【1, 2】 + 【3, 3】 = 6
// 09:16.702 [ForkJoinPool-1-worker-1] - join: 【1, 3】 + 【4, 5】 = 15
// 09:16.703 [main] - 15
```
...
public class Test18 {
    ...
    //=> 41:09.533 [  t1] - locking...
    //=> 41:09.539 [  t1] - 睡眠 1 秒
    //=> 41:10.549 [  t1] - unlocking...
    //=> 41:10.549 [  t3] - locking...
    //=> 41:10.549 [  t3] - 睡眠 1 秒
    //=> 41:11.556 [  t3] - unlocking...
    //=> 41:11.556 [  t2] - locking...
    //=> 41:11.556 [  t2] - 睡眠 1 秒
    //=> 41:12.568 [  t2] - unlocking...
}
...
### 8.9. JUC - ReentrantLock 原理

![image-20260210120612173](./images/image-20260210120612173.png)
...
![image-20260210121152781](./images/image-20260210121152781.png)

第一个竞争者（Thread-1）出现:

![image-20260210121525263](./images/image-20260210121525263.png)
...
![image-20260210122016827](./images/image-20260210122016827.png)

Thread-1 执行 acquireQueued() 的逻辑:
...
![image-20260210122800551](./images/image-20260210122800551.png)

再次有多个线程经历上述过程竞争失败，变成这个样子:

![image-20260210123137343](./images/image-20260210123137343.png)
...
![image-20260210123235356](./images/image-20260210123235356.png)
...
![image-20260210123622337](./images/image-20260210123622337.png)

如果这时候有其它线程来竞争（非公平的体现），例如这时有 Thread-4 来了:

![image-20260210124009854](./images/image-20260210124009854.png)
...
thread-0 第一次上锁: state 从 0 变为 1, 加锁成功

thread-0 第二次上锁: state 从 1 变为 2

thread-0 第一次释放锁: state 从 2 变为 1

thread-0 第二次释放锁: state 从 1 变为 0, 释放锁成功
...
* 获得锁后，再自己执行一次 `Thread.currentThread().interrupt();`

可打断模式:

* 被打断时，会抛异常，不会再循环尝试获取锁

#### 8.9.4. 公平锁原理

非公平锁: 如果没有上锁，不看 等待队列，直接去竞争锁
...
公平锁: 如果没有上锁，先看 等待队列，如果等待队列为空 才去竞争锁
...
### 8.10. JUC - 读写锁 - reentrantreadwritelock
...
public class Test19 {
    public static void main(String[] args) {
        ...
        // 读-读 可以并发
        new Thread(() -> dc.read(), "t1").start();
        new Thread(() -> dc.read(), "t2").start();

        // 读-写 互斥
        new Thread(() -> dc.read(), "t3").start();
        new Thread(() -> dc.write(), "t4").start();

        // 写-写 互斥
        new Thread(() -> dc.read(), "t5").start();
        new Thread(() -> dc.write(), "t6").start();
    }
}
...
重入时升级不支持：即 持有读锁 的情况下 去获取写锁，会导致获取写锁永久等待

重入时降级支持：即 持有写锁 的情况下 去获取读锁，是支持的
...
* 更新时，先 清缓存 再 更新数据库: 更新数据库比较耗时，这段空隙里 有很大概率 缓存又被填充旧数据
* 更新时，先 更新数据库 再 清缓存
...
public class Test20 {
    public static void main(String[] args) {
        ...
        UserService userService = new UserCachedService();

        userService.queryUserById(1);
        userService.queryUserById(1);
        userService.updateUser(1, "张三三");
        userService.queryUserById(1);
    }
}
...
@Slf4j(topic = "c.Test")
class UserService {
    private final Map<Integer, String> mockDb = new Hashtable<>();
    ...
    public String queryUserById(Integer id) {
        String name = mockDb.get(id);
        log.debug("查询: SELECT * FROM tb_user WHERE id = {} \t\t结果: {}", id, name);
        return name;
    }

    public void updateUser(Integer id, String name) {
        log.debug("更新: UPDATE tb_user SET name = {} WHERE id = {}", name, id);
        mockDb.put(id, name);
    }
}

class UserCachedService extends UserService {
    UserService userService = new UserService();

    // 缓存
    Map<Integer, String> cache = new Hashtable<>();
    ...
    public String queryUserById(Integer id) {
        ...
        try {
            ...
            String value = userService.queryUserById(id);
            cache.put(id, value);
...
* 当线程释放了 permits 就像离开了停车场，停车场显示空余车位加一
* 如果停车场满了，则需要在入口排队等待
...
public class Test16 {
    ...
    //=> 33:01.324 [  t1] - running...
    //=> 33:01.324 [  t3] - running...
    //=> 33:01.328 [  t3] - 睡眠 1.0 秒
    //=> 33:01.328 [  t1] - 睡眠 1.0 秒
    //=> 33:02.339 [  t1] - end...
    //=> 33:02.339 [  t3] - end...
    //=> 33:02.340 [  t5] - running...
    //=> 33:02.340 [  t4] - running...
    //=> 33:02.340 [  t5] - 睡眠 1.0 秒
    //=> 33:02.340 [  t4] - 睡眠 1.0 秒
    //=> 33:03.346 [  t4] - end...
    //=> 33:03.346 [  t5] - end...
    //=> 33:03.346 [  t2] - running...
    //=> 33:03.346 [  t2] - 睡眠 1.0 秒
    //=> 33:04.350 [  t2] - end...
}
```
...
* 用 Semaphore 实现简单连接池，对比『享元模式』下的实现（用wait notify），性能和可读性显然更好， 注意 线程数和数据库连接数是相等的

原理:

* 创建对象: 构造函数中的 permits 参数，就是 state 的数值
* 上锁: 每次上锁都会让 state 减一；当 state 为 0 时，后续线程都会进入 AQS 等待队列
* 解锁: 每次解锁都会让 state 加一

### 8.13. JUC - countdownlatch
...
public class Test23 {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 1; i <= 2 ; i++) {
            new Thread(() -> {
                ...
                sleepSeconds(new Random().nextInt(3));
    ...
    //=> 51:32.557 [main] - wait begin
    //=> 51:32.557 [  t1] - task begin
    //=> 51:32.557 [  t1] - task begin
    //=> 51:32.560 [  t1] - 睡眠 1.0 秒
    //=> 51:32.560 [  t1] - 睡眠 2.0 秒
    //=> 51:33.565 [  t1] - task end
    //=> 51:34.575 [  t1] - task end
    //=> 51:34.575 [main] - wait end
}
```
...
public class Test24 {
    ...
    //=> 55:08.633 [pool-1-thread-2] - task begin
    //=> 55:08.633 [pool-1-thread-1] - task begin
    //=> 55:08.633 [pool-1-thread-3] - wait begin
    //=> 55:08.636 [pool-1-thread-1] - 睡眠 0.0 秒
    //=> 55:08.637 [pool-1-thread-1] - task end
    //=> 55:08.636 [pool-1-thread-2] - 睡眠 1.0 秒
    //=> 55:09.650 [pool-1-thread-2] - task end
    //=> 55:09.650 [pool-1-thread-3] - wait end
}
```

#### 8.13.3. 应用 - 等待多线程准备完毕

模拟英雄联盟 10 个应用加载完毕进入游戏
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 10 ; i++) {
            ...
            pool.submit(() -> {
                ...
                for (int j = 0; j <= 100; j++) {
                    progresses[index] = j + "%";
                    sleepMilliseconds(random.nextInt(100));

                    // 打印进度
                    // \r 可以回退到行首，覆盖当前行之前的打印
...
public class Test26 {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 1; i <= 4; i++) {
            new Thread(() -> {
                log.debug("begin");
                sleepMilliseconds(random.nextInt(1000));
                log.debug("end");

                try {
                    // 计数减 1, 减到 0 后，执行一次汇总，然后重置为 2
                    cyclicBarrier.await();
    ...
    //=> 11:37.376 [  t4] - end
    //=> 11:37.653 [  t3] - end
    //=> 11:37.653 [  t3] - 执行完 2 次，进行汇总
    //=> 11:37.877 [  t1] - end
    //=> 11:37.916 [  t2] - end
    //=> 11:37.916 [  t2] - 执行完 2 次，进行汇总
}
```
...
比如: Hashtable 、 Vector，所有读写方法都使用 synchronized 修饰

#### 8.15.2. 使用 synchronizedXxx 方法装饰的线程安全集合

说明:

* 通过装饰器模式，将传入的 不安全集合 变为安全的集合
* 也是使用 synchronized 修饰 集合的通用方法
...
java.util.concurrent.* 下的线程安全集合类，它们的类名有规律，有三类关键词：
...
CopyOnWrite* 之类容器修改开销相对较重，用于读多写少的情况
...
log.debug("map: {}", map);
//=> {0=4427, 1=6880, 2=4557, 3=4646, 4=4600, 5=3901, 6=4301, 7=2942, 8=5560, 9=3914}
```
    ...
    threadList.add(new Thread(() -> {
        synchronized (map) {
            for (int i = 0; i < 1000; i++) {
                String key = num % max + "";
                // 如果不存在 则 计算
                LongAdder longAdder = map.computeIfAbsent(key, (k) -> new LongAdder());
                longAdder.increment();
            }
        }
    }));
...
#### 8.16.4. JDK 7 - HashMap 并发死链

HashMap:

* 初始是 长度为 8 的数组，元素为 key 的链表
* 通过 hash(key) % 8 获取下标（桶下标），将 key 放入对应 桶下标 的链表
* 当 key 的容量达到 map 容量的 四分之三 时，会发生扩容（翻倍）
* 当查找元素时，先找对应桶下标的链表，再在链表中通过 `key.equals(other)` 来找对应的节点
...
```java
// initialCapacity(比如 9) 除以 loadFactor(默认 0.75) 得到 12
// 将 12 处理成 2的N次方，即最终的实际容量为 16
...
put 时，key 或 value 为 null 时会报空指针异常
...
### 8.17. LinkedBlockingQueue

阻塞队列

略

### 8.18. ConcurrentLinkedQueue

并发队列

略

### 8.19. CopyOnWriteArrayList

CopyOnWriteArraySet 是它的马甲 底层实现采用了 写入时拷贝 的思想，

增删改操作会将底层数组拷贝一份，更改操作在新数组上执行，
