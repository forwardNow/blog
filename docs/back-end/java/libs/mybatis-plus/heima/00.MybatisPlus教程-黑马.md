...
@modify date 2025-12-08 00:20:36
@desc [description]
#endregion-->
...
* 数据库中 json 类型的字段 与 PO 中属性的转换
* json 类型 本质上存的还是 JSON 字符串，PO 中使用 String 类型的变量就可以接收

需求:

* PO 中使用 实体来接收 json 类型的数据

步骤:

1. 设置 JSON 转换器
   * 在 PO 的对象类型变量上通过 `@TableField(typeHandler = JacksonTypeHandler.class)` 设置转换器
2. 开启 autoResultMap
   * 在 PO 的类上通过 `@TableName(value = 数据库表名, autoResultMap = true)` 开启嵌套对象的自动映射处理
...
@Data
// MyBatis 中，对象中嵌套对象，需要写 resultMap
// 如果不想写，开启 autoResultMap
@TableName(value = "user", autoResultMap = true)
public class User {
    // ......

    // 该字段使用 JacksonTypeHandler 处理器来进行转换
...
void testPageQuery() {
    ...
    ==>  Preparing: SELECT id, ... FROM user ORDER BY balance DESC, id ASC LIMIT ?
    total = 5
    pages = 3
    */
}
...
public PageDTO<UserVO> queryUsersPage(UserQuery query) {
    ...
    boolean isAsc = BooleanUtil.isTrue(query.getIsAsc());
...
public class PageQuery {
    ...
    public <T> Page<T> toMpPage(OrderItem ... defaultOrderItems) {
        ...
        } else if (defaultOrderItems != null) {
            // sortBy 为空，则使用默认排序
...
public class PageDTO<T> {
    ...
    public static <PO, VO> PageDTO<VO> of(Page<PO> page, Class<VO> clazz) {
        PageDTO<VO> dto = getPageDTO(page);
        ...
        // 4. PO 转 VO
        dto.setList(BeanUtil.copyToList(poList, clazz));
    ...
    public static <PO, VO> PageDTO<VO> of(Page<PO> page, Function<PO, VO> convertor) {
        ...
        // 4. PO 转 VO
        List<VO> voList = poList.stream().map(convertor).collect(Collectors.toList());
    ...
    private static <PO, VO> PageDTO<VO> getPageDTO(Page<PO> page) {
