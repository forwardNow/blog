...
@modify date 2026-03-09 19:09:50
@desc [description]
#endregion-->
...
![image-20260226192958512](./images/image-20260226192958512.png)
...
[聚合（aggregations）](https://www.elastic.co/docs/explore-analyze/query-filter/aggregations)可以实现对文档数据的统计、分析、运算。
...
```js
// # 嵌套聚合 metric
// # 按品牌进行分桶，桶里再根据 评分 求平均值等
// # 对桶进行排序: 按 桶内 评分的平均分 进行倒序排序
GET /hotel/_search
{
  "size": 0, 
  "aggs": {
...
![image-20260303122753170](./images/image-20260303122753170.png)
...
![image-20260303122901386](./images/image-20260303122901386.png)
...
在 IUserService 中定义方法，实现对品牌、城市、星级的聚合
...
public Map<String, List<String>> filters() {
    try {
        ...
        result.put("品牌", getAggByName("brandAgg", aggregations));
        result.put("城市", getAggByName("cityAgg", aggregations));
        result.put("星级", getAggByName("starAgg", aggregations));
...
private List<String> getAggByName(String aggName, Aggregations aggregations) {
...
可以看到请求参数与之前 search 时的 RequestParam 完全一致，这是在限定聚合时的文档范围。

例如：用户搜索“外滩”，价格在 300~600，那聚合必须是在这个搜索条件基础上完成。

因此我们需要：
1. 编写 controller 接口，接收该请求
2. 修改 IUserService#getFilters() 方法，添加 RequestParam 参数
3. 修改 getFilters 方法的业务，聚合时添加 query 条件

### 12.11. 自动补全

#### 12.11.1. 安装拼音分词器

比如: 搜索框输入 “sj” 可以匹配到 “手机”、“书籍” 等关键词

要实现根据字母做补全，就必须对文档按照拼音分词。

在 GitHub 上恰好有 elasticsearch 的拼音分词插件。

地址：https://github.com/infinilabs/analysis-pinyin
...
1. character filters：在 tokenizer 之前对文本进行处理。例如 删除字符、替换字符
...
![image-20260303191637755](./images/image-20260303191637755.png)
...
如果 搜索时也使用 拼音 分词器，会匹配到很多同义词，比如 搜索 “狮子” 会匹配到 “虱子”
...
elasticsearch 提供了 [Completion Suggester](https://www.elastic.co/guide/en/elasticsearch/reference/7.6/search-suggesters.html#completion-suggester) 查询来实现自动补全功能。
...
注意：name、all 是可分词的，自动补全的 brand、business 是不可分词的，要使用不同的分词器组合
...
#### 12.11.5. RestAPI实现自动补全查询

先看请求参数构造的 API：

![image-20260304195920112](./images/image-20260304195920112.png)
...
![image-20260305124025648](./images/image-20260305124025648.png)

**方案二：异步通知 【推荐】**

![image-20260305124220795](./images/image-20260305124220795.png)

**方案三：监听 binlog**

![image-20260305124454920](./images/image-20260305124454920.png)
...
1. 导入课前资料提供的 hotel-admin 项目，启动并测试酒店数据的 CRUD
2. 声明 exchange、queue、RoutingKey
3. 在 hotel-admin 中的增、删、改业务中完成消息发送
4. 在 hotel-demo 中完成消息监听，并更新 elasticsearch 中数据
5. 启动并测试数据同步功能

#### 12.12.3. 声明队列和交换机

![image-20260305183457517](./images/image-20260305183457517.png)
...
![image-20260305215055888](./images/image-20260305215055888.png)

#### 12.13.2. 搭建集群

我们计划利用 3 个 docker 容器模拟 3 个 es 的节点。


我们会在单机上利用 docker 容器运行多个 es 实例来模拟 es 集群。

不过生产环境推荐大家每一台服务节点仅部署一个 es 的实例。

部署 es 集群可以直接使用 docker-compose 来完成，但这要求你的 Linux 虚拟机至少有 4G 的内存空间

**创建 es 集群**

首先编写一个 docker-compose 文件，内容如下: (`/tmp/es-cluster/docker-compose`)
...
kibana 可以监控 es 集群，不过新版本需要依赖 e s的 x-pack 功能，配置比较复杂。

这里推荐使用 cerebro 来监控 es 集群状态，官方网址：https://github.com/lmenezes/cerebro

解压 cerebro-0.9.4.zip 后，进入 bin 目录双击执行 cerebro.bat

管理界面: http://localhost:9000/

连接任意 ES 节点，比如: http://192.168.124.10:9200

**创建索引库**

方式 1: 利用 kibana 的 DevTools 创建索引库
...
| coordinating | 上面3个参数都为false则为coordinating节点 | 无 | 路由请求到其它节点 |
| 合并其它节点处理的结果，返回给用户 |
...
默认情况下，每个节点都是 master eligible 节点，因此一旦 master 节点宕机，其它候选节点会选举一个成为主节点。

当主节点与其他节点网络故障时，可能发生脑裂问题。

为了避免脑裂，需要要求选票超过 `( eligible节点数量 + 1 ）/ 2` 才能当选为主，因此 eligible 节点数量最好是奇数。
