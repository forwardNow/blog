...
@modify date 2026-04-28 15:20:16
@desc [description]
#endregion-->
...
export class ZipUtil {
   ...
   */
  private async compress(folderPath: string, zipFilePath: string) {
    // 创建文件写入流
   ...
   */
  private async decompress(zipFilePath: string, outputFolderPath: string) {
    try {
      fs.mkdirSync(outputFolderPath, { recursive: true }); // 确保目录存在

      await decompress(zipFilePath, outputFolderPath);

      console.log(`解压完成: ${zipFilePath}`);
      return true;
    } catch (err) {
      console.error(`解压失败: ${zipFilePath}`, err);
      return false;
    }
  }

  private unzip(zipFilePath: string, outputFolderPath: string) {
    try {
      fs.mkdirSync(outputFolderPath, { recursive: true }); // 确保目录存在
  ...
  private zipFolder(folderPath: string, zipFilePath: string) {
