...
@modify date 2025-12-09 14:22:56
@desc [description]
#endregion-->
...
2. 一旦 mousedown，则在元素上注册 mousemove 和 mouseup，并记录元素初始的位置和鼠标按下的位置
...
1. 在 容器 元素上侦听 mousedown 事件

2. 一旦 mousedown，判断 event.target 是否为目标元素，不是则返回

3. 在 容器 元素上注册 mousemove 和 mouseup，并记录元素初始的位置和鼠标按下的位置
   * 在 mousemove 里改变元素的位置
   * 在 mouseup 里注销 mousemove 和 mouseup 事件
   * 移动时超过 容器 的范围，则注销 mousemove 和 mouseup 事件
