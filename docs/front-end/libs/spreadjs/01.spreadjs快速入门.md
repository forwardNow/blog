...
@modify date 2026-05-11 10:50:41
@desc [description]
#endregion-->
...
```js
// SpreadJs 默认使用英文资源，所以无需载入英文资源，这里直接载入需要切换的中文资源
...
```js
var sheet = GC.Spread.Sheets.Designer.findControl(document.getElementById('gc-designer-container')).getWorkbook().getActiveSheet();

// 获取存在 数据或公式 的已使用区域
// null | IRange
sheet.getUsedRange(GC.Spread.Sheets.UsedRangeType.data)
//=> {row: 1, rowCount: 2, col: 1, colCount: 2}
```

参考: [获取range中真实使用的区域](https://demo.grapecity.com.cn/spreadjs/help/docs/faqs/worksheet/other/used-range)
...
```js
GC.Spread.Sheets.productInfo.productVersion
```
...
const csvData = sheet.getCsv(
  0,                          // 起始行索引
  0,                          // 起始列索引
  sheet.getRowCount(),        // 总行数
  sheet.getColumnCount(),     // 总列数
  "\r\n",                     // 行分隔符（换行符）
  ","                         // 列分隔符（逗号）
);
```
...
```js
worksheet.getRange(row, col, rowCount, colCount).backColor("red");

// 透明色
worksheet.getRange(row, col, rowCount, colCount).backColor("");
```
...
```js
const styleJson = sheet.getStyle(0, 0).toJSON()
...
Cell Editors -> Cell Type : 通过 instanceof 判断
...
cellType.fromJSON(cellTypeJson);

sheet.getCell(0, 1).cellType(cellType)
```

单元格类型: https://demo.grapecity.com.cn/spreadjs/help/docs/features/cells/celltypes
...
var fbx = new GC.Spread.Sheets.FormulaTextBox.FormulaTextBox(
    // 指定了一个 DOM 元素作为公式文本框的容器
    document.getElementById('formulaBar'), 
    { 
      // absoluteReference : 是否使用绝对引用。
      // needSheetName : 是否在引用中显示工作表名称
...
// 公式文本框绑定到 Spread 工作簿实例，使其能够访问工作簿中的工作表数据
// 如果不传参数调用此方法，则会解除绑定。
fbx.workbook(spread)
...
参考: [SpreadJS -> 示例 -> 功能示例 -> 公式计算 -> 公式编辑框 -> 区域选择器](https://demo.grapecity.com.cn/spreadjs/SpreadJSTutorial/features/calculation/formula-textbox/range-select/purejs)
...
// 判断第 2 行是否可见
const isRowVisible = !sheet.getRowVisible(1); 
// 判断第 3 列是否可见
const isColVisible = !sheet.getColumnVisible(2);
```
...
* 如果 valuePath 为 `'dataUrl'`，则 `cell.value()` 返回 base64 编码的内容
* 将 valuePath 设置为 `undefined`，则 `value()` 返回 IFileInfo
...
if (cellType instanceof GC.Spread.Sheets.CellTypes.FileUpload) {
  cellType.valuePath(undefined);
...
```js
export class LwShowImage extends GC.Spread.CalcEngine.Functions.AsyncFunction {
  constructor() {
    super('LwShowImage', 0, 0, {
      description: 'Cross workbook reference',
      parameters: [],
    });
  }
  ...
  evaluateAsync(context, ...args: string[]) {
    const { ctx } = context;
    const sheet = ctx.source.getSheet();

    // imageUrl 可以是 base64
    const imgUrl = 'https://cdn.grapecity.com.cn/online/%E7%AB%8B%E5%8D%B3%E4%B8%8B%E8%BD%BD_spjs.png';

    const result = GC.Spread.Sheets.CalcEngine.evaluateFormula(sheet, `=IMAGE("${imgUrl}")`, 0, 0);
...
参考: https://demo.grapecity.com.cn/spreadjs/help/api/modules/GC.Spread.Sheets.CalcEngine#evaluateformula
...
// 当前 单元格
var cell = GC.Spread.Sheets.Designer.findControl(document.getElementById('gc-designer-container')).getWorkbook().getActiveSheet().getCell(sheet.getActiveRowIndex(), sheet.getActiveColumnIndex())
```
