...
@modify date 2026-04-27 22:47:49
@desc [description]
#endregion-->
...
作用是限制后续环境变量、命令扩展或延迟变量扩展的修改范围，仅作用于当前批处理脚本（.bat/.cmd）内部，避免“污染”全局环境

从 setlocal 执行起，所有对环境变量的修改仅在当前批处理脚本内有效。

当遇到 endlocal 或脚本结束时，系统会自动恢复到 setlocal 之前的环境状态。
...
# 如果 1.txt 文件存在， 则将其拷贝到 2.txt
if exist D:\dev\1.txt (copy /y D:\dev\1.txt D:\dev\2.txt)
```
