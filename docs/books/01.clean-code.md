...
### 1.7. Avoid Mental Mapping (避免思维映射)
...
#### 1.7.5. Pick One Word per Concept (一个概念一个词)
...
#### 1.7.6. Don’t Pun (不要使用双关语)

比如， `add` 方法，有的表示将数值相加，有的表示将新元素添加到集合，
这种同一个方法名表示多个意思的就是双关

#### 1.7.7. Use Solution Domain Names (使用解决方案相关的名称)
...
#### 1.7.8. Use Problem Domain Names (使用业务相关的名称)

如果不能使用程序员熟悉的术语，就采用业务相关的名称，至少可以去请教业务代表。

#### 1.7.9. Add Meaningful Context (添加有意义的上下文)
...
#### 1.7.10. Don’t Add Gratuitous Context (不要随意添加上下文前缀)
...
### One Level of Abstraction per Function (一个函数就是一个抽象层级)
